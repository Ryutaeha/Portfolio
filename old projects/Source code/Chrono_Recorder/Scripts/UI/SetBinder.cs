    ...
    public class Binder
    {
        ...
        public void Binding<T>(GameObject parentObject) where T : Object
        {
            ...
            foreach (var comp in objects)
            {
                ...
                if (!objectDict.ContainsKey(key)) objectDict.Add(key, comp as Object);
                else Debug.Log($"중복 key 발견 :{key}");
        ...
        // UnBinding 메소드는 _objects 딕셔너리에서 주어진 타입의 데이터를 제거합니다.
        // 이 메소드는 _objects 딕셔너리에 저장된 데이터 중에서 타입이 T와 일치하는 데이터를 제거합니다.
        public void UnBinding<T>() where T : Object
        {
            if (_objects.ContainsKey(typeof(T))) _objects.Remove(typeof(T));
            else Debug.Log($"{typeof(T).Name} 타입의 데이터가 존재하지 않습니다.");
        }
